...
// Use same-origin relative API calls. The browser sends requests to the UI
// server, which proxies them to the Cognee backend container internally.
const localApiUrl = "";
...
export default async function localFetch(url: URL | RequestInfo, options: RequestInit = {}): Promise<Response> {
  ...
  const fullUrl = urlStr === "/health" || urlStr.startsWith("/health?")
    ? urlStr.replace("/health", "/api/health")
